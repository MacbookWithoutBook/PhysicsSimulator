...
public class AccountFragment extends Fragment implements View.OnClickListener {
    ...
    private void createAccount() {
		...
		// If a user Account already exists, do not insert the account into database
		if (mUserAccountViewModel.hasUserName(username)){
		...
		if (username.matches(".*(?=.*( |\n|\t|\r)).*")) {
			mEtUsername.setError("Username cannot have whitespace.");
			return;
		}

		if (password.equals(confirm) && !TextUtils.isEmpty(username) && !TextUtils.isEmpty(password) && passwordIsValid(password)) {
			...
			mEtPassword.setError("Password too short (Must contain at least 6 characters).");
		} else if (!password.matches(".*(?=.*[a-z])(?=.*[A-Z]).*")) {
			mEtPassword.setError("Password must contain at least one uppercase and one lowercase letter.");
	...
	public static boolean passwordIsValid(String password) {
		// Passwords must be of length 6 or greater and contain an uppercase and lowercase letter.
		return password.length() >= 6 && password.matches(".*(?=.*[a-z])(?=.*[A-Z]).*");
	}
}
